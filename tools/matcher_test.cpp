...
static std::shared_ptr<Item> mk(const std::string& s, size_t idx = 0) {
    return std::make_shared<Item>(s, idx);
}
...
static void stress(AlgoType algo, const char* name) {
    ...
    for (int it = 0; it < ITER; ++it) {
        ...
        auto item = mk(text);
        MatchResult r = m.match(item, pat);
...
int main() {
    ...
    // ---- --exact mode flips
    {
        Matcher me(CaseMode::Smart, AlgoType::FuzzyV2, true);
    ...
    // ---- unsortable queries keep input order (fzf's sortable flag)
    {
        ...
        auto items2 = std::vector<std::shared_ptr<Item>>{
